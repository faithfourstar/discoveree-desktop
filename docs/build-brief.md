...
| Sources | Provenance layer — users audit what agents believe. **Includes market-aware source discovery (confirmed 4 Aug 2026):** the SaaS reads the product's markets (`products.markets` — ported) to find locale-relevant review/news sites and mine in the market's language (`analyzeReviewPlatforms`, `discoverNewsPublications`, `getMultiLingualContext` — deferred in sprints 2 and 3b; they land with the Sources sprint, alongside doc upload from §4a and source re-validation). Note: market language is orthogonal to display locale — a German-market product mines German sites whatever language the UI speaks. | `SourcesTab.tsx`, `searchProviders.ts` |
...
2. **MCP write surface (wording amended 4 Aug 2026 to match the ADR 004 gate ruling):** MCP tools (`propose_competitor_intel`, `log_feedback`) so the customer's AI (e.g. Claude reading their Slack) can push intel in. The gate follows the seat and the object, not the transport: **owner-seat `log_feedback` is a direct add** (feedback is pooled evidence — influence only through aggregation; `mcp` provenance with required attribution), while **competitor intel ALWAYS queues** for accept (it lands on a record as if fact; `intel_proposals` → review block in the owning module + home digest note). Reader-seat writes refuse and discard (content is never retained — retaining would make readers writers with the owner as rubber stamp). **Routing principle:** team knowledge enters via the paid seat's AI harvesting shared channels, with human attribution ("shared by Maya in #sales · via Claude") — the reader refusal never loses org intel, because the knowledge still sits in the channel where the owner's harvest reaches it. Belongs to the MCP sprint (5a); connect-a-teammate + per-reader revocable credentials are 5b. This is also the reader-upgrade moment mechanism. Consumers: Claude, Cursor, ChatGPT, and GitHub Copilot (VS Code — spawns local MCP servers like Cursor, day-one); Microsoft 365 Copilot is cloud-side and reaches only remote connectors — a team-tier consumer, same class as claude.ai-in-browser (hedge public claims until verified).
...
Schema already supports org → many products with product-scoped context (protected by ADR 001's tenancy decision); the current single-product API surface is a convenience layer, not a constraint.

**Brand layer (recorded 4 Aug 2026, deferred by design):** some orgs have brands between company and products (e.g. Salesforce acquiring Intercom, which itself has products). Own side: a `brands` table + nullable `products.brandId` is purely additive (context hangs off products, so nothing downstream moves); build it with the Commercial Model/portfolio sprint, where brand roll-ups actually mean something (investment/revenue by brand), plus switcher grouping and later brand-scoped team-tier access. Competitor side: track the acquired brand (Intercom) as its own root entity with its products as children and `parentCompany` attribution ("Salesforce") — usually the right model; the two-level hierarchy invariant lives in service code, not SQL, so deepening it stays a design decision if real demand appears. The open design (ADR 003) is **cross-product entity identity**: org-level canonical entities (a competitor as a company, researched once; a segment/persona) with product-scoped facets (threat level and feature comparison per product; jobs-to-be-done per product even when the persona is shared). Must be settled before the Customer Insights port. Team tier will also need per-product access control back (leaders all-product; product teams theirs).
