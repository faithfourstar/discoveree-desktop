...
4. **Discard of a proposed facet** (DELETE while `proposed`): delete the facet; if **no other facet references the entity node**, delete the node — and, if that was the last child of a company node that itself has no facets, the company node too — along with their change rows, preserving §9's "a proposal that was never accepted leaves no history". If other facets exist anywhere on the tree, only the discarded facet goes: Product A's tracked context is never collateral damage of Product B's discard.
...
3. The proposal card presents the resolution for confirmation: *"Xero is a company with several products — you appear to compete with Xero Payroll."* The user can confirm the suggested child, switch to another listed child, or choose company grain ("compete with Xero as a whole"). This is ordinary §9 mechanics — nothing is tracked until accept, and the §2.3 discard rule GCs the whole proposed tree (root + unfaceted children) if abandoned. Sibling child nodes created during resolution but never faceted are kept as **identity-only rows** (names + urls, no enrichment spend): a later add from any of our products matches them instantly, they cost nothing, and they are invisible outside the org entity view.
