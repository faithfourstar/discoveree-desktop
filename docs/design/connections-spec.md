...
5. **QR code** (120px, right of the address block) encoding the network address — caption 12px: *"for getting the address onto another device quickly."* No claims that scanning "connects" anything.
...
- Both themes: amber never-heard-from stamps, destructive port/poll failures, `VIA CLAUDE` chips, teal just-connected moments, QR block — all pass in light and dark (`colour/5` + `colour/20` tinted surfaces; QR gets a white quiet-zone card in dark mode).
...
- **Honest-connection audit:** no state or copy claims "connected" before a query is received; waiting states carry no pulse; the aged state blames nothing.
