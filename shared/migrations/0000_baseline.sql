-- 0000_baseline — full desktop schema, squashed (no SaaS migration history).
-- Generated by drizzle-kit from shared/schema.ts, then reconciled against the
-- raw SQL DDL in the SaaS repo's server/db.ts ensureSchemaColumns()
-- (ADR 001 risk 6). Tables that came from that reconciliation (they existed
-- ONLY in the raw SQL, never in the SaaS shared/schema.ts):
--   * roadmap_recommendations        (Roadmap Review & Suggestions state)
--   * deleted_customer_segment_names (segment deletion blocklist)
-- Indexes folded in from the same raw SQL: idx_market_reviews_product,
-- idx_market_reviews_product_month (unique), idx_idea_assessments_user_product,
-- idx_thought_partner_conversations_org/_product, idx_shared_conversations_org,
-- idx_threat_level_history_profile/_product, idx_call_recordings_segment,
-- idx_deleted_segments_product (+ unique product/normalized_name).
-- Everything else in the raw DDL either duplicates shared/schema.ts or belongs
-- to cut modules and is deliberately absent. The ensureSchemaColumns() /
-- runOneTimeMigrations() patterns themselves are banned in this repo
-- (ADR 001 §3): schema changes are ordinary numbered migrations here.
...
CREATE TABLE "competitor_profiles" (
	...
	"parent_company" text,
	"battlecard_messages" jsonb,
	"battlecard_ready_flag" boolean DEFAULT false,
	"battlecard_output" jsonb,
